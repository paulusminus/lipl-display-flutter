...
fn get_options_from_cargo() -> RawOpts {
    ...
    let rust_input = format!("{}/{}", current_dir.as_os_str().to_string_lossy(), rust_input_option);
    let dart_output = format!("{}/{}", current_dir.parent().unwrap().as_os_str().to_string_lossy(), dart_output_option);
